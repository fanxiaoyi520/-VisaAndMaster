...
- (void)viewDidLoad {
    ...
    if ([self.payInfo containsString:@"https"]) {
        //https
        NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:self.payInfo]];
...
- (void)getDataFromNetWorkingUnifiedQuery:(_Nullable id)info {
    ...
    [[ZDPayNetRequestManager sharedSingleton] zd_netRequestVC:self Params:dic postUrlStr:[NSString stringWithFormat:@"%@%@",DOMAINNAME([[ZDPay_OrderSureModel sharedSingleten] getModelData].urlStr),UNIFIEDQUERY] suscess:^(id  _Nullable responseObject) {
        ...
        if ([[responseObject objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            if ([status isEqualToString:@"22"] || [status isEqualToString:@"2"] || [status isEqualToString:@"1"]) {
//22就是交易正在处理中
//23 交易已批准
                //返回上个界面
                if (self.webViewBlock) self.webViewBlock(responseObject,NO);
                [self.navigationController popViewControllerAnimated:YES];
            } else if ([status isEqualToString:@"23"]){
...
- (void)viewDidLoad {
    ...
    [self getDataFromNetWorkingPreOrderCheck];
}
...
//预下单校验 PREORDERCHECK
- (void)getDataFromNetWorkingPreOrderCheck{
    @WeakObj(self)
    NSDictionary *dic = [[ZDPayFuncTool sharedSingleton] getPreOrderCheckDictionary];
    [[ZDPayNetRequestManager sharedSingleton] zd_netRequestVC:self Params:dic postUrlStr:[NSString stringWithFormat:@"%@%@",DOMAINNAME([[ZDPay_OrderSureModel sharedSingleten] getModelData].urlStr),PREORDERCHECK] suscess:^(id  _Nullable responseObject) {
        @StrongObj(self)
        if ([[responseObject objectForKey:@"code"] isEqualToString:@"200"]) {
            [[ZDPayInternationalizationModel sharedSingleten] setModelProcessingDic:[responseObject objectForKey:@"data"]];
            self.naviTitle = [[ZDPayInternationalizationModel sharedSingleten] getModelData].PAYMENT;
            ...
            NSMutableDictionary *dic = [ZDPayFuncTool getPayResultDicToClientWithCode:@"1001" withData:[responseObject objectForKey:@"data"] withMessage:@"预下单校验失败"];
            self.completionBlock(dic);
        }
    }];
}
